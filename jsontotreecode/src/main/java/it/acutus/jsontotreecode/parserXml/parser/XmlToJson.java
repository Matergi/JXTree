...
public class XmlToJson {
    ...
    public static String afterTag = "¶%&%&¶";
    public static String preTag = "%&¶¶%&";
    public static String array = "_@array@_";

    private ArrayList<String> tagExists = new ArrayList<>();
    ...
    private void readTags(Tag parent, XmlPullParser xpp, int precedentLevel) {
        try {
            int eventType;
            do {
                eventType = xpp.next();
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    int level = xpp.getDepth();

                    String tagName = xpp.getName();

                    String pathGeneral = parent.getPath();

                    if (tagExists.contains(pathGeneral + "/" + tagName))
                    {
                        ...
                        {
                            count += 1;
                            tagName = firstTagName + afterTag + count;
                        } while (tagExists.contains(pathGeneral + "/" + tagName));
                    }

                    tagExists.add(pathGeneral + "/" + tagName);

                    String path = parent.getPath() + "/" + tagName;
                    Tag child = new Tag(path, tagName);
                    parent.addChild(child);

                    // Attribute are taken into account as key/values in the child
